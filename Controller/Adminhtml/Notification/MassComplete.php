...
class MassComplete extends ParentClass
{
    public function execute()
    {
        ...
        foreach ($collection as $item) {
            $item->setStatus(NotificationInterface::STATUS_COMPLETE)
                ->setUserId($this->_authSession->getUser()->getId())
                ->save();
        }

        $this->messageManager->addSuccessMessage(__('A total of %1 record(s) have been set to complete.', $collectionSize));
